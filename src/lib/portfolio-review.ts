...
export async function generateReview(notes: string | null): Promise<PortfolioReviewRecord> {
  ...
  try {
    ...
    const [recentFilings, weakenedImpacts, riskImpacts] = await Promise.all([
      db.filing.findMany({
        where: { filingDate: { gte: sevenDaysAgo }, filingType: { in: ["10-K", "10-Q", "8-K"] } },
        orderBy: { filingDate: "desc" },
        take: 10,
        include: { thesisImpacts: { orderBy: { createdAt: "desc" }, take: 1 } },
      }),
      db.thesisImpactRecord.findMany({
        where: { impactLevel: { in: ["weakened", "kill_criteria_triggered"] }, createdAt: { gte: sevenDaysAgo } },
    ...
    if (filingsRequiringReview.length === 0) {
      filingsRequiringReview = [{ ticker: null, headline: "No new filings in last 7 days", detail: "Run ingestion to check for new SEC filings.", severity: "info" }];
    }

    thesisAlerts = weakenedImpacts.map(t => ({
      ticker: t.ticker,
      headline: t.impactLevel === "kill_criteria_triggered" ? `Kill criteria triggered: ${t.ticker}` : `Thesis weakened: ${t.ticker}`,
      detail: t.reasoning,
    ...
    if (thesisAlerts.length === 0) {
      thesisAlerts = [{ ticker: null, headline: "No thesis alerts", detail: "No weakening or kill-criteria signals detected in recent filings.", severity: "low" }];
    }
    ...
    if (newRisksDetected.length === 0) {
      newRisksDetected = [{ ticker: null, headline: "No new risks detected", detail: "No new risk signals from SEC filings in the past 30 days.", severity: "low" }];
    }
  } catch {
