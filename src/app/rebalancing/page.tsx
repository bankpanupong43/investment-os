...
import type { AllocationResponse, AllocationEntry, UntrackedPosition } from "@/app/api/allocation/route";
    ...
    <tr className="border-b border-[#EEEEEE] hover:bg-[#F4F4F4]" style={{ transition: "background-color 0.2s" }}>
      ...
      <td className="px-5 py-3.5">
        <div className="flex items-center gap-2">
          ...
          {detail && (
            <span className="text-xs px-1.5 py-0.5 rounded border text-[#2d7d46] bg-[#eef7f1] border-[#c3e6cf] hidden sm:inline">
              ● Portfolio
            </span>
          )}
        </div>
        ...
        {detail && (
          <div className="text-xs text-[#8E8E8E] mt-0.5 tabular-nums">
            {detail.shares.toLocaleString()} sh
            {detail.price != null && <> · {fmt(detail.price)}</>}
          </div>
        )}
      </td>
      ...
      <td className="px-5 py-3.5 text-right tabular-nums">
        ...
        <div className="text-xs text-[#8E8E8E]">{t.targetPct.toFixed(0)}% target</div>
      </td>
      ...
      <td className="px-5 py-3.5 text-right tabular-nums">
        ...
        {t.currentUsd > 0 && (
          <div className="text-xs text-[#8E8E8E]">{t.currentPct.toFixed(1)}% of portfolio</div>
        )}
      </td>
      ...
      <td className="px-5 py-3.5 hidden md:table-cell">
        <div className="flex items-center gap-2">
          <div className="relative w-24 h-1.5 bg-[#EEEEEE] rounded-full overflow-hidden">
            ...
            />
            <div className="absolute top-0 h-full w-0.5 bg-[#171A20] opacity-30" style={{ left: "100%", transform: "translateX(-50%)" }} />
          </div>
      ...
      {/* Gap */}
      <td className="px-5 py-3.5 text-right tabular-nums">
        {needsBuy && <div className="text-[#b45309] font-medium">−{fmtK(t.gapUsd)}</div>}
        {isOverfund && <div className="text-[#2d7d46] font-medium">+{fmtK(Math.abs(t.gapUsd))}</div>}
        {!needsBuy && !isOverfund && <span className="text-[#2d7d46] text-xs font-medium">✓ met</span>}
      </td>

      {/* Action */}
      <td className="px-5 py-3.5">
        {needsBuy && (
          <span className="text-xs px-2 py-0.5 rounded font-medium uppercase text-[#b45309] bg-[#fffbeb] border border-[#fde68a]">buy</span>
        )}
        {isOverfund && (
          <span className="text-xs px-2 py-0.5 rounded font-medium uppercase text-[#5C5E62] bg-[#F4F4F4] border border-[#EEEEEE]">hold</span>
        )}
        {!needsBuy && !isOverfund && (
          <span className="text-xs px-2 py-0.5 rounded font-medium uppercase text-[#2d7d46] bg-[#eef7f1] border border-[#c3e6cf]">funded</span>
        )}
      </td>
    </tr>
  );
...
function UntrackedRow({ u, detail }: { u: UntrackedPosition; detail: HoldingDetail | null }) {
  return (
    <tr className="border-b border-[#EEEEEE] hover:bg-[#F4F4F4]" style={{ transition: "background-color 0.2s" }}>
      <td className="px-5 py-3.5 hidden lg:table-cell" />
      <td className="px-5 py-3.5">
        <div className="flex items-center gap-2">
          {u.positionId && u.positionId !== u.ticker ? (
            <Link href={`/positions/${u.positionId}`} className="font-medium text-[#171A20] hover:text-[#3E6AE1]" style={{ transition: "color 0.2s" }}>
              {u.ticker}
            </Link>
          ) : (
            <span className="font-medium text-[#171A20]">{u.ticker}</span>
          )}
          <span className={`text-xs px-1.5 py-0.5 rounded border capitalize hidden sm:inline ${BUCKET_COLOR[u.bucket] ?? "text-[#5C5E62] bg-[#F4F4F4] border-[#EEEEEE]"}`}>
            {u.bucket}
          </span>
          <span className="text-xs px-1.5 py-0.5 rounded border text-[#2d7d46] bg-[#eef7f1] border-[#c3e6cf] hidden sm:inline">
            ● Portfolio
        ...
        <div className="text-xs text-[#8E8E8E] mt-0.5 truncate max-w-[160px]">{u.name}</div>
        {detail && (
          <div className="text-xs text-[#8E8E8E] mt-0.5 tabular-nums">
            {detail.shares.toLocaleString()} sh
            {detail.price != null && <> · {fmt(detail.price)}</>}
      ...
      <td className="px-5 py-3.5 text-right tabular-nums">
        <div className="text-xs text-[#8E8E8E]">no target</div>
      </td>
      <td className="px-5 py-3.5 text-right tabular-nums">
        <div className="text-[#171A20] font-medium">{fmtK(u.currentUsd)}</div>
        <div className="text-xs text-[#8E8E8E]">{u.currentPct.toFixed(1)}% of portfolio</div>
      </td>
      <td className="px-5 py-3.5 hidden md:table-cell" />
      <td className="px-5 py-3.5" />
      <td className="px-5 py-3.5">
...
function CandidateRow({ ticker, onRemove }: { ticker: string; onRemove: () => void }) {
  return (
    <tr className="border-b border-[#EEEEEE] hover:bg-[#F4F4F4]" style={{ transition: "background-color 0.2s" }}>
      <td className="px-5 py-3.5 hidden lg:table-cell" />
      <td className="px-5 py-3.5">
        <div className="flex items-center gap-2">
          <span className="font-medium text-[#171A20]">{ticker}</span>
          <span className="text-xs px-1.5 py-0.5 rounded border text-[#5C5E62] bg-[#F4F4F4] border-[#EEEEEE] hidden sm:inline">
            ○ Candidate
          </span>
        </div>
        <div className="text-xs text-[#8E8E8E] mt-0.5">Not currently owned</div>
      </td>
      <td className="px-5 py-3.5 text-right tabular-nums">
        <div className="text-xs text-[#D0D1D2]">—</div>
      </td>
      <td className="px-5 py-3.5 text-right tabular-nums">
      ...
      <td className="px-5 py-3.5">
        <button
          onClick={onRemove}
          className="text-xs text-[#8E8E8E] hover:text-[#c0392b]"
...
export default function RebalancingPage() {
  ...
  const [holdingMap, setHoldingMap] = useState<Map<string, HoldingDetail>>(new Map());
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch("/api/portfolio-value").then(r => r.json()).catch(() => ({ holdings: [] })),
    ])
      .then(([alloc, pv]) => {
        if (alloc.error) { setError(alloc.error); return; }
        setData(alloc as AllocationResponse);
        const map = new Map<string, HoldingDetail>();
        for (const h of (pv.holdings ?? []) as { ticker: string; shares: number; price: number | null }[]) {
          map.set(h.ticker, { ticker: h.ticker, shares: h.shares, price: h.price });
        }
        setHoldingMap(map);
      })
  ...
  function handleAddCandidate(e: React.FormEvent) {
    e.preventDefault();
    const ticker = candidateInput.trim().toUpperCase();
    if (!ticker || candidates.includes(ticker) || holdingMap.has(ticker)) return;
    setCandidates(prev => [...prev, ticker]);
    setCandidateInput("");
  }
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 max-w-7xl">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-medium text-[#171A20]">Buy Planner</h1>
          <p className="text-[#8E8E8E] text-sm mt-0.5">
            Live prices · {fmtDate(summary.snapshotDate)} · {settings.exchangeRate.toFixed(2)} THB/USD
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <div
              ...
              className={`w-8 h-4 rounded-full transition-colors relative ${showAllHoldings ? "bg-[#3E6AE1]" : "bg-[#D0D1D2]"}`}
            >
              <div className={`absolute top-0.5 w-3 h-3 rounded-full bg-white shadow transition-transform ${showAllHoldings ? "translate-x-4" : "translate-x-0.5"}`} />
            </div>
            <span className="text-sm text-[#5C5E62]">Show Current Holdings</span>
          </label>
          <Link
            href="/holdings"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard
          ...
          sub={`฿${(settings.totalCapitalThb / 1000).toFixed(0)}K live total`}
        />
        <MetricCard
          label="Deployed to Targets"
          value={fmtK(summary.totalDeployedUsd)}
          sub={`${summary.pctFunded.toFixed(1)}% of target plan funded`}
        />
        <MetricCard
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white border border-[#EEEEEE] rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-[#EEEEEE] flex items-center justify-between gap-4 flex-wrap">
            <div>
              <h2 className="font-medium text-[#171A20] text-sm">Target Allocation</h2>
              <p className="text-xs text-[#8E8E8E] mt-0.5">
                {targets.length} targets
                {showAllHoldings && untracked.length > 0 && ` · ${untracked.length} in portfolio`}
            ...
            <form onSubmit={handleAddCandidate} className="flex items-center gap-2">
              <input
                ...
                onChange={e => setCandidateInput(e.target.value.toUpperCase())}
                placeholder="Add candidate..."
                className="text-xs border border-[#EEEEEE] rounded px-2 py-1.5 w-28 focus:outline-none focus:border-[#3E6AE1] bg-[#F4F4F4]"
              />
              <button
                type="submit"
                className="text-xs bg-[#F4F4F4] border border-[#EEEEEE] rounded px-2.5 py-1.5 font-medium text-[#5C5E62] hover:bg-[#EEEEEE] shrink-0"
              ...
              <tbody>
                {targets.map((t, i) => (
                  <AllocationRow
                    ...
                    t={t}
                    rank={i + 1}
                    detail={holdingMap.get(t.ticker) ?? null}
                  />
                ))}
                {showAllHoldings && untracked.map(u => (
                  <UntrackedRow
                    key={u.ticker}
                    u={u}
                    detail={holdingMap.get(u.ticker) ?? null}
                  />
                ))}
                {candidates.map(ticker => (
                  <CandidateRow
                    key={ticker}
                    ticker={ticker}
                    onRemove={() => setCandidates(prev => prev.filter(t => t !== ticker))}
                  />
                ))}
              </tbody>
              <tfoot>
                <tr className="bg-[#F4F4F4] border-t border-[#EEEEEE]">
                  ...
                  <td className="px-5 py-3 text-xs text-[#8E8E8E] font-medium">
                    TOTAL ({targets.length} targets{candidates.length > 0 ? ` · ${candidates.length} candidates` : ""})
                  </td>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white border border-[#EEEEEE] rounded-xl overflow-hidden">
            <div className="px-5 py-4 border-b border-[#EEEEEE]">
              ...
              <p className="text-xs text-[#8E8E8E] mt-0.5">ranked by gap · cash {fmtK(summary.cashUsd)}</p>
            </div>
            {buyCandidates.length === 0 ? (
              ...
              buyCandidates.map((t, i) => {
                const detail = holdingMap.get(t.ticker) ?? null;
                return (
                  <div key={t.ticker} className={`px-5 py-3.5 ${i < buyCandidates.length - 1 ? "border-b border-[#EEEEEE]" : ""}`}>
                    ...
                    {/* Gap breakdown */}
                    <div className="text-xs text-[#8E8E8E] space-y-0.5 mb-2">
                      <div className="flex justify-between">
                        <span>Target</span>
                        <span className="tabular-nums text-[#171A20]">{fmtK(t.targetUsd)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Current</span>
                        <span className="tabular-nums text-[#171A20]">{fmtK(t.currentUsd)}</span>
                      </div>
                      {detail && (
                        <div className="flex justify-between">
                          <span>Holdings</span>
